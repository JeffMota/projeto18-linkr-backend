...
import { emailCheck, userRegistry } from "../repository/auth.repository.js"
dotenv.config()
...
export async function signup(req, res) {
    ...
    try {
        ...
        const alreadyExist = await emailCheck(email)

        if (alreadyExist.rows.length > 0) return res.sendStatus(409)

        const hashedPassword = bcrypt.hashSync(password, 10)

        userRegistry(username, email, hashedPassword, picture_url)
